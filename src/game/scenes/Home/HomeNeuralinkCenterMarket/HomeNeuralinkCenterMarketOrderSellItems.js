...
import { container_center_market_order_sell_sub } from "./HomeNeuralinkCenterMarketOrderSell.js";

let container_main = null;

let isOpen = false;

export function CreateNeuralinkCenterMarketOrderSellItems(scene) {
    Destroy(scene);

    //console.log("CreateCenterMarketOrderBuyItems");
...
function RequestOrderSellList(scene) {
    ...
    centerData.RequestNeuralinkCenterMarketItemOrderBookSell(
        (result) => {
            HideLoadingPopup();
...
function CreateItemList(scene, receivedData) {
    ...
    for (let i = 0; i < receivedData.data.orders.length; i++) {
        let indexData = receivedData.data.orders[i];
        ...
        container_item.button_cancel.button.on("pointerdown", function () {
            CreateAlertPopup(
                ...
                () => {
                    ...
                    centerData.RequestNeuralinkCenterMarketOrderCancel(
                        indexData._id,
                        () => {
                            HideLoadingPopup();
    ...
    item.text_name = scene.add
        .text(
            ...
            cdLocalization.getLocalization(
                ...
                item.itemData.assetIdentifier.replaceAll("_", " ")
            ),
            {
...
function CreateSell(scene, item) {
    //console.log("CreateSell item: ", item);
    ...
    if (item.itemData.itemType) {
        itemCode = item.itemData.itemType.code;
    } else if (item.itemData.fragmentType) {
        itemCode = `${item.itemData.fragmentType.characterCode}_fragment_${item.itemData.fragmentType.fragmentOrder}`;
    }
    ...
    itemIcon.setPosition(540, 194 + 500 / 2);
    ...
    container_sell_buttons.add(btn_close);

    CreateSellPriceList(scene, item);
}

let container_price_list = null;

function CreateSellPriceList(scene, item) {
    ...
    //container_list.setDepth(200);
    container_sell.add(container_price_list);

    // Kích thước của ScrollView
    const scrollViewWidth = 1200;
    const scrollViewHeight = 610;
    ...
    for (let i = 0; i < item.itemData.listingList.length; i++) {
        let indexData = item.itemData.listingList[i];

        let container_item = CreateSellPriceItem(
            scene,
            scrollablePanel,
            indexData
        );
        ...
        container_item.button_cancel.button.on("pointerdown", function () {
            CreateAlertPopup(
                ...
                () => {
                    ...
                    centerData.RequestCenterMarketSellCancel(
                        indexData.id,
                        () => {
                            HideLoadingPopup();
...
function CreateSellPriceItem(scene, scrollablePanel, itemData) {
    let itemWidth = 1000;
    let itemHeight = 110;
