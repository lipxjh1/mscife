...
export function CreateChipToMSCIConvertPopup(
    scene,
    convertRate,
    remainingConvertibleChip,
    totalChipToConvert,
    onSuccess,
    onFailed
) {
    ...
    askStr += `\n${remainingConvertibleChip}/${totalChipToConvert}`;
    ...
    btn_yes.button.on("pointerdown", function () {
        let chipToConvert = Number(inputValue);

        if (chipToConvert > centerData.userInfo.Chip) {
            CreateAlertPopup(
                scene,
                cdLocalization.getLocalization(
                    cdLocalization.GROUP_KEYS.HomeWallet.KEY,
        ...
        if (chipToConvert != null && chipToConvert > 0) {
            let caculatedMSCI = chipToConvert * convertRate;

            CreateAlertPopup(
                scene,
                cdLocalization.getLocalization(
                    cdLocalization.GROUP_KEYS.HomeWallet.KEY,
                    `Convert {i} Chip to {i} $MSCI`,
                    [chipToConvert, caculatedMSCI]
                ),
                () => {
                    CreateLoadingPopup();

                    centerData.RequestMSCIConvert(
                        chipToConvert,
                        (result) => {
                            HideLoadingPopup();

                            if (onSuccess && typeof onSuccess === "function") {
                                onSuccess(result, chipToConvert);
                            }
