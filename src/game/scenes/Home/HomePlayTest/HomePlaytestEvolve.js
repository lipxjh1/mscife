...
import {
    CreateCardOptions,
    CloseCardOptions,
} from "./HomePlaytestCharacterEvolve.js";
...
export function CreatePlayTestEnvolved(scene, _id) {
    characterId = _id;

    let unlockedPlayer = centerDataPlayer.playTestPlayer[characterId];
...
function SetSelectedCharacterInfo(scene) {
    ...
    const text_availableCount = scene.add
        .text(547 + 336 / 2, 507, amountToUpgrade + "/" + amountToUpgrade, {
            fontFamily: cdLocalization.getCurrentFont(),
            fontSize: "128px",
            color: "#ffffff",
            align: "center",
...
function RequestEnvolved(scene, unlockedPlayer) {
    CreateAlertPopup(scene, "Evolve success");
