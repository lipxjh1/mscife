...
import { CreatePlayTestUpStar } from "./HomePlaytestUpgrade.js";
...
export function CreatePlaytestCharacterUpStar(scene) {
    Destroy();
...
function AssetsLoadDone(scene) {
    ...
    let btn_play = scene.add
        .image(615 + 330 / 2, 58 + 90 / 2, "home_character_btn_upstartest")
        .setOrigin(0.5, 0.5);
    ...
    centerDataPlayer.CreatePlayTestStarPlayerList();
...
function CreateItemList(scene) {
    ...
    let playerDict = centerDataPlayer.playTestStarPlayer;
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (
            pData !== null &&
            centerDataPlayer.avoidRankToUpStar.includes(playerDict[k].rank) ==
                false
        ) {
            const newItem = {
...
export function CreateCardOptions(scene, _id) {
    ...
    let unlockedPlayer = centerDataPlayer.playTestStarPlayer[_id];
    ...
    btn_upgrade.button.on("pointerdown", function () {
        if (centerData.selectedPlayerArr.includes(item.unlockedPlayer._id)) {
            CreateAlertPopup(
                scene,
                cdLocalization.getLocalization(
                    cdLocalization.GROUP_KEYS.HomeTeam.KEY,
            ...
            CreatePlayTestUpStar(scene, item.unlockedPlayer._id);
        }
    });
}
