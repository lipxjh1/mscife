...
export function CreateCenterMarketSellCharacterStar(scene, roleKey, codeKey) {
...
export function getGroupCodePlayers() {
    return groupCodePLayers;
}
...
function CreateCharacterList(scene) {
    ...
    let codeGroup = getGroupRolePlayers()[selectedRole][selectedCode];

    let groupStarKeys = Object.keys(codeGroup);

    for (let i = 0; i < groupStarKeys.length; i++) {
        if (i >= 4) {
            break;
        }

        let groupStarKey = groupStarKeys[i];

        //console.log("groupStarKey: ", groupStarKey);

        let groupStar = codeGroup[groupStarKey];

        //console.log("groupStar: ", groupStar);
...
function CreateItem(scene, scrollablePanel, itemData) {
    ...
    for (const level in item.itemData.groupStar) {
        const characterCodes = item.itemData.groupStar[level];
...
export function Close(scene) {
    ...
    CreateCenterMarketSellCharacterCode(scene, selectedRole);
}
