...
import { CreateCenterMarketSellItemsDetail } from "./HomeCenterMarketSellItemsDetail.js";
...
export function CreateCenterMarketSellItems(scene) {
...
function CreateItemList(scene) {
    ...
    let inventoryItem = GetGroupSellItems();
    ...
    for (let i = 0; i < itemCodes.length; i++) {
        ...
        let itemInventory = inventoryItem[itemCode];
        ...
        if (itemLocalData != null && quantity > 0) {
            const indexData = {
            ...
            container_item.button_sell.button.on("pointerdown", function () {
                CreateCenterMarketSellItemsDetail(scene, itemCode);
            });
        }
    }
...
function CreateItem(scene, scrollablePanel, itemData) {
    ...
    item.text_quantity = scene.add
        .text(
            ...
            ) + item.itemData.itemInventory.quantity,
            {
                fontFamily: cdLocalization.getCurrentFont(),
                fontSize: "38px",
                color: "#ffffff",
