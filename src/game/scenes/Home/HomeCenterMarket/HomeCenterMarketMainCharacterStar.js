...
import { CreateCenterMarketCharacterCode } from "./HomeCenterMarketMainCharacterCode.js";

import { GetRoleContainerMain } from "./HomeCenterMarketMainCharacterRole.js";
...
let characterStarLevelStatistics = {};

export function CreateCenterMarketCharacterStar(scene, roleKey, codeKey) {
    //console.log("CreateCenterMarketCharacterLevel");
...
export function RequestCharacterStarLevelStatisticsDetail(onSuccess, onError) {
    CreateLoadingPopup();
    ...
    centerData.RequestGetCMarketCharacterListingStatistics(
        ...
        (result) => {
            ...
            characterStarLevelStatistics = result.data[0];
...
export function GetCountByStar(star) {
    let statistics = characterStarLevelStatistics["stars"][star];

    let count = 0;
    for (let k = 0; k < 10; k++) {
        let level = k + 1;

        let levelStatistics = statistics["levels"][level];

        count += levelStatistics.totalListCount;
    }

    return count;
}

export function GetCountByStarLevel(star, level) {
    let statistics = characterStarLevelStatistics["stars"][star];
    let levelStatistics = statistics["levels"][level];
    return levelStatistics.totalListCount;
}
...
export function Close(scene) {
    ...
    CreateCenterMarketCharacterCode(scene, selectedRole);
}
...
export function Destroy(scene) {
    ...
    //console.log("HomeCenterMarketMainCharacterStar Destroy");
}
