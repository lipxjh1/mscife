...
import { CreateCenterMarketItems } from "./HomeCenterMarketMainItems.js";

let itemCode = null;

export function CreateCenterMarketItemsDetail(scene, selectedItemCode) {
...
function CreateBuy(scene) {
    ...
    let baseItemData = centerData.baseItemInfo[itemCode];
    ...
    centerData.RequestGetCMarketItemListing(
        itemCode,
        currentPage,
        PAGE_LIMIT,
        (result) => {
    ...
    btn_cancel.button.on("pointerdown", function () {
        ...
        CreateCenterMarketItems(scene, itemCode);
    });
}
...
function createPriceListItem(scene, itemWidth, itemHeight) {
    ...
    btn_select.button.on("pointerdown", function () {
        ...
        CreateAlertPopup(
            scene,
            `Buy this item with ${itemData.totalPrice} M-Coin?`,
            () => {
                if (itemData.totalPrice > centerData.userInfo.Musk) {
                    CreateAlertPopup(
                        scene,
                        cdLocalization.getLocalization(
                ...
                CreateLoadingPopup();
                centerData.RequestPostCMarketItemBuyByListId(
                    itemData.listingId,
                    itemData.quantity,
                    (result) => {
                        HideLoadingPopup();
    ...
    container.updateContent = function (data) {
        ...
        text_count.setText("Listed: " + data.quantity);
        text_seller.setText(
            data.seller && data.seller.username ? data.seller.username : ""
        );
    };
