...
import { CreateCenterMarketItemsPieceAllDetail } from "./HomeCenterMarketMainItemsPieceAllDetail.js";
import {
    CreateCenterMarketItemsPieceCode,
    GetFragmentByCode,
    UpdateFragmentByCode,
} from "./HomeCenterMarketMainItemsPieceCode.js";
import { GetItemTypeContainerMain } from "./HomeCenterMarketMainItemsType.js";
...
export function CreateCenterMarketItemsPieceAll(scene, codeKey) {
    //console.log("CreateCenterMarketItemsPieceAll codeKey: ", codeKey);
...
function RequestBuyItemList(scene) {
    ...
    let requestPieceKeys = [];

    for (let i = 0; i < 4; i++) {
        let pieceKey = `${selectedCode}_fragment_${i + 1}`;

        pieceKeys.push(pieceKey);

        requestPieceKeys.push(pieceKey.toLocaleUpperCase());
    }
    ...
    centerData.RequestGetCMarketItemListingStatistics(
        ...
        (result) => {
            ...
            for (let i = 0; i < result.data.length; i++) {
                ...
                UpdateFragmentByCode(itemCode, itemData);
            }
...
function CreateItemList(scene) {
    ...
    for (let i = 0; i < pieceKeys.length; i++) {
        let pieceKey = pieceKeys[i];

        //console.log("pieceKey: ", pieceKey);

        let pieceData = GetFragmentByCode(pieceKey.toLocaleUpperCase());

        let itemLocalData = centerDataItem.getItemById(pieceKey);

        let baseInfo = centerData.baseItemInfo[pieceKey];

        if (itemLocalData != null && pieceData.totalListings > 0) {
            const indexData = {
                pieceKey: pieceKey,
                pieceData: pieceData,
            ...
            container_item.button_buy.button.on("pointerdown", function () {
                CreateCenterMarketItemsPieceAllDetail(
                    scene,
                    selectedCode,
                    pieceKey
                );
    ...
    item.text_quantity = scene.add
        .text(
            ...
            cdLocalization.getLocalization(
                ...
                ": " +
                item.itemData.pieceData.totalListings,
            {
                fontFamily: cdLocalization.getCurrentFont(),
                fontSize: "38px",
                color: "#ffffff",
...
export function Close(scene) {
    ...
    CreateCenterMarketItemsPieceCode(scene);
}
