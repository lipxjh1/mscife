...
import { CreateCenterMarketSellItemsPieceAllDetail } from "./HomeCenterMarketSellItemsPieceAllDetail.js";
...
export function CreateCenterMarketSellItemsPieceAll(scene, codeKey) {
...
function CreateItemList(scene) {
    ...
    let codeGroup = GetGroupSellItemsPiece()[selectedCode];
    ...
    let pieceKeys = Object.keys(codeGroup);
    ...
    for (let i = 0; i < pieceKeys.length; i++) {
        ...
        if (itemLocalData != null && pieceData && pieceData.quantity > 0) {
            ...
            container_item.button_sell.button.on("pointerdown", function () {
                CreateCenterMarketSellItemsPieceAllDetail(scene, pieceKey);
            });
        }
    }
...
function CreateItem(scene, scrollablePanel, itemData) {
    ...
    item.text_quantity = scene.add
        .text(
            ...
            ) + item.itemData.pieceData.quantity,
            {
                fontFamily: cdLocalization.getCurrentFont(),
                fontSize: "38px",
                color: "#ffffff",
