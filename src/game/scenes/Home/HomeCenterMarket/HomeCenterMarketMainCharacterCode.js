...
import { CreateCenterMarketCharacterStar } from "./HomeCenterMarketMainCharacterStar.js";
...
export function CreateCenterMarketCharacterCode(scene, roleKey) {
...
function RequestBuyCharacterList(scene) {
    ...
    centerData.RequestGetCMarketCharacterTradeAble(
        (result) => {
            ...
            centerData.RequestGetCMarketCharacterListingStatistics(
                allowTradeCharacterCodes,
                false,
                (result) => {
                    HideLoadingPopup();
                    ...
                    for (let i = 0; i < result.data.length; i++) {
                        ...
                        characterStatistics[code] = result.data[i];
                    }
...
export function GetAllowTradeCharacterCodes() {
    return allowTradeCharacterCodes;
}

export function IsAllowTradeCharacterCode(code) {
    return allowTradeCharacterCodes.includes(code);
}
...
function CreateCharacterList(scene) {
    ...
    container_list.add(container_character_list);
    ...
    for (let i = 0; i < allowTradeCharacterCodes.length; i++) {
        let charCode = allowTradeCharacterCodes[i];

        let baseInfo = centerData.baseCharacterInfo[charCode];

        let pData = centerDataPlayer.getPlayerById(charCode);

        let statistics = characterStatistics[charCode];

        if (pData != null && statistics.totalListCount > 0) {
            let indexData = {
                charCode: charCode,
                pData: pData,
                baseInfo: baseInfo,
            ...
            container_item.button_buy.button.on("pointerdown", function () {
                CreateCenterMarketCharacterStar(scene, selectedRole, charCode);
    ...
    item.text_quantity = scene.add
        .text(
            ...
            cdLocalization.getLocalization(
                ...
                ": " +
                item.itemData.statistics.totalListCount,
            {
                fontFamily: cdLocalization.getCurrentFont(),
                fontSize: "38px",
                color: "#ffffff",
...
export function Destroy(scene) {
    ...
    //console.log("HomeCenterMarketMainCharacterCode Destroy");
}
