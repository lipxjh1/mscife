...
import { container_menu_buttons } from "./HomeCenterMarket.js";

import {
    CreateCharacterList as CreateRoleCharacterList,
    getGroupRolePlayers,
    GetRoleContainerMain,
} from "./HomeCenterMarketSellCharacterRole.js";

import { CreateCenterMarketSellCharacterStar } from "./HomeCenterMarketSellCharacterStar.js";
...
let allowTradeCharacterCodes = [];

export function CreateCenterMarketSellCharacterCode(scene, roleKey) {
    ...
    GetRoleContainerMain().add(container_main);
    ...
    RequestBuyCharacterList(scene);

    Open(scene);
}

function RequestBuyCharacterList(scene) {
    CreateLoadingPopup();

    centerData.RequestGetCMarketCharacterTradeAble(
        (result) => {
            HideLoadingPopup();
            ...
            for (let i = 0; i < result.data.length; i++) {
                ...
                if (tempArr.includes(code) == false && role == selectedRole) {
                    tempArr.push(code);
                }
            }
            allowTradeCharacterCodes = tempArr;
...
function CreateCharacterList(scene) {
    ...
    let roleGroup = getGroupRolePlayers()[selectedRole];

    let groupCodeKeys = Object.keys(roleGroup);

    for (let i = 0; i < groupCodeKeys.length; i++) {
        let groupCodeKey = groupCodeKeys[i];

        // console.log("groupCodeKey: ", groupCodeKey);

        if (
            allowTradeCharacterCodes.includes(groupCodeKey) == true &&
            groupCodeKey.includes(centerDataPlayer.RANK_KEY.sc.KEY) == false &&
            groupCodeKey.includes(centerDataPlayer.RANK_KEY.sb.KEY) == false &&
            groupCodeKey.includes(centerDataPlayer.RANK_KEY.sa.KEY) == false
        ) {
            let groupCode = roleGroup[groupCodeKey];

            //console.log("groupCode: ", groupCode);

            let baseInfo = centerData.baseCharacterInfo[groupCodeKey];

            //console.log("baseInfo: ", baseInfo);

            let pData = centerDataPlayer.getPlayerById(groupCodeKey);
            ...
            if (pData !== null) {
                ...
                container_item.button_sell.button.on(
                    "pointerdown",
                    function () {
                        CreateCenterMarketSellCharacterStar(
                            scene,
                            selectedRole,
                            groupCodeKey
                        );
...
function CreateItem(scene, scrollablePanel, itemData) {
    ...
    let container_card = CreateCharacterCard(
        scene,
        "",
        item.itemData.baseInfo.code,
        item.itemData.baseInfo.name,
        item.itemData.baseInfo.role,
        item.itemData.baseInfo.rank,
        0,
        0
    );
    ...
    container_card.setPosition(28 + 150 / 2, 33 + 150 / 2);

    container_inner.add(container_card);

    item.text_name = scene.add
        .text(238, 33, item.itemData.baseInfo.name, {
    ...
    for (const starLevel in item.itemData.groupCode) {
        const levels = item.itemData.groupCode[starLevel];
