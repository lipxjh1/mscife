...
export function CreateCenterMarketHistoryOrderCharacter(scene) {
    //console.log("CreateCenterMarketHistoryOrderCharacter");
...
function RequestOrderBuyList(scene) {
    ...
    centerData.RequestGetCMarketCharacterMyBuy(
        currentPage,
        PAGE_LIMIT,
        (result) => {
            HideLoadingPopup();
...
function createOrderItem(scene, itemWidth, itemHeight) {
    ...
    const text_seller_id = scene.add
        .text(1010, 55, "", {
            fontFamily: cdLocalization.getCurrentFont(),
            fontSize: "30px",
            color: "#ffffff",
        ...
        .setOrigin(1, 0);
    container_inner.add(text_seller_id);
    ...
    container.updateContent = function (data) {
        ...
        text_orderId.setText(data.orderId);
        text_id.setText(data._id);

        text_seller_id.setText(data.seller._id);
