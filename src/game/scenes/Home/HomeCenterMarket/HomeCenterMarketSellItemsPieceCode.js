...
import { CreateCenterMarketSellItemsPieceAll } from "./HomeCenterMarketSellItemsPieceAll.js";

import {
    CreateCenterMarketSellItemsType,
    GetGroupSellItemsPiece,
...
export function CreateCenterMarketSellItemsPieceCode(scene) {
...
function RequestToSellList(scene) {
    CreateLoadingPopup();
    ...
    centerData.RequestInventory(
        (result) => {
            ...
            //console.log("centerData.RequestInventory result: ", result);
...
function CreateItemList(scene) {
    ...
    let itemDict = GetGroupSellItemsPiece();

    let codeKeys = Object.keys(itemDict);

    let allowItemCodes = GetTradeAbleItems();

    for (let i = 0; i < codeKeys.length; i++) {
        let codeKey = codeKeys[i];

        if (allowItemCodes.includes(codeKey.toUpperCase() + "_FRAGMENT_1")) {
            //console.log("codeKey: ", codeKey);

            let groupData = itemDict[codeKey];

            //console.log("groupData: ", groupData);

            let pieceKeys = Object.keys(groupData);

            let count = 0;

            for (let k = 0; k < pieceKeys.length; k++) {
                let tempItemLocalData = centerDataItem.getItemById(
                    pieceKeys[k]
                );

                if (tempItemLocalData != null) {
                    count += groupData[pieceKeys[k]].quantity;
                }
            }

            // console.log("pieceKeys: ", pieceKeys);

            let itemLocalData = centerDataItem.getItemById(pieceKeys[0]);

            let baseCharInfo = centerData.baseCharacterInfo[codeKey];

            let baseInfo = centerData.baseItemInfo[pieceKeys[0]];

            if (itemLocalData != null && count > 0) {
                const indexData = {
                    codeKey: codeKey,
                    baseCharInfo: baseCharInfo,
                    groupData: groupData,
                ...
                container_item.button_sell.button.on(
                    "pointerdown",
                    function () {
                        CreateCenterMarketSellItemsPieceAll(scene, codeKey);
...
function CreateItem(scene, scrollablePanel, itemData) {
    ...
    item.text_name = scene.add
        .text(
            ...
            item.itemData.baseCharInfo.name +
                " " +
