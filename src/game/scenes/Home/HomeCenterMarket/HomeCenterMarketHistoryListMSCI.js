...
export function CreateCenterMarketHistoryListMSCI(scene) {
    //console.log("CreateCenterMarketHistoryListMSCI");
...
function createHistoryItem(scene, itemWidth, itemHeight) {
    ...
    container.updateContent = function (data) {
        ...
        // Cập nhật thông tin cơ bản theo schema mới
        // coinType: loại coin (MSCI), amount: số lượng, pricePerUnit: giá mỗi đơn vị
        const amount = typeof data.amount === "number" ? data.amount : 0;
        const pricePerUnit =
            typeof data.pricePerUnit === "number" ? data.pricePerUnit : 0;
        const totalPrice = amount * pricePerUnit;

        // Tên mục: hiển thị loại coin và đơn giá
        text_name.setText(`${data.coinType || "$MSCI"} ${pricePerUnit}`);

        // Tổng giá (M-Coin)
        text_price.setText("M-Coin: " + totalPrice);

        // Thời điểm tạo và mã listing
        text_create_time.setText(formatDateTime(data.createdAt));
        text_id.setText(data.listingId);
        ...
        // Cập nhật số lượng (Quantity -> Amount)
