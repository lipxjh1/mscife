...
export function CreateCenterMarketHistoryOrderMSCI(scene) {
    //console.log("CreateCenterMarketHistoryOrderMSCI");
    ...
    // Empty text sẽ được hiển thị trong CreateCharacterList nếu không có dữ liệu
...
function RequestOrderBuyList(scene) {
    ...
    centerData.RequestGetCMarketMSCIPurchased(
        currentPage,
        PAGE_LIMIT,
        (result) => {
            HideLoadingPopup();
...
function CreateCharacterList(scene, receivedData) {
    ...
    if (!receivedData || !receivedData.data || receivedData.data.length === 0) {
        const emptyText = scene.add
            .text(540, 600, "No MSCI Transaction History", {
...
function createOrderItem(scene, itemWidth, itemHeight) {
    ...
    const text_price_unit = scene.add
        .text(238, 75, "", {
            fontFamily: cdLocalization.getCurrentFont(),
            fontSize: "30px",
            color: "#FFA600",
        ...
        .setOrigin(0, 0);
    container_inner.add(text_price_unit);
    ...
    container.updateContent = function (data) {
        ...
        // Cập nhật thông tin cơ bản theo cấu trúc dữ liệu mới
        text_name.setText(data.type || "UNKNOWN");

        text_price.setText(data.totalPrice + " M-Coin");

        text_price_unit.setText(data.pricePerUnit + " M-Coin/Unit");
        ...
        // Hiển thị orderId từ listingId hoặc transactionId
        if (data.listingId && data.listingId.orderId) {
            text_order_id.setText("Order: " + data.listingId.orderId);
        } else {
            text_order_id.setText("Transaction: " + data.transactionId);
        }
        ...
        // Cập nhật thông tin người bán từ sellerId
        if (data.sellerId && data.sellerId.Username) {
            text_seller.setText("Seller: " + data.sellerId.Username);
            text_seller.setVisible(true);
        } else if (data.buyerId && data.buyerId.Username) {
            text_seller.setText("Buyer: " + data.buyerId.Username);
            text_seller.setVisible(true);
        } else {
