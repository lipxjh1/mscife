...
export function CreateUpgrade(scene, _id) {
    characterId = _id;
...
function SetSelectedCharacterInfo(scene) {
    ...
    if (unlockedPlayer.nextLevelProperties.attachDamage) {
        text_current_damage.setText(
            text_current_damage.text +
                " => " +
                unlockedPlayer.nextLevelProperties.attachDamage
        );
    }
    ...
    if (unlockedPlayer.nextLevelProperties.attackDelay) {
        text_current_delay.setText(
            text_current_delay.text +
                " => " +
                unlockedPlayer.nextLevelProperties.attackDelay
        );
    }
...
function CreateSelectCharacterItemList(scene) {
    ...
    let btn_1_UnlockedPlayer = centerData.getUnlockedPlayerById(characterId);

    console.log("btn_1_UnlockedPlayer:", btn_1_UnlockedPlayer);
...
function RequestUpgrade(scene, unlockedPlayer) {
    ...
    centerData.RequestCharactersUpStar(
        characterOfUserIds,
        ...
        (err) => {
            ...
            CreateAlertPopup(scene, "Up star failed\n" + err.message);
        }
    );
}
