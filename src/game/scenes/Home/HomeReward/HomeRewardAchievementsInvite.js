...
export let container_invite_list = null;
let maskShape = null;
let mask = null;
let isOpen = false;

// Container for the whole achievement panel, imported from the main file
import { container_achievements_sub_panel } from "./HomeRewardAchievements.js";

export function CreateInviteList(scene) {
    if (isOpen) return;
    isOpen = true;
    ...
    centerData.RequestAchievementsInfo(
        ...
        (result) => {
            ...
            container_invite_list = scene.add.container(0, 0);
            container_achievements_sub_panel.add(container_invite_list);

            if (!arr_data || arr_data.length <= 0) {
                //console.log("No invite achievements data available");
                const text = scene.add
                    ...
                    .setOrigin(0.5, 0.5);
                container_invite_list.add(text);
                return;
            }
            ...
            const scrollablePanel = scene.rexUI.add
                ...
                .layout();
            container_invite_list.add(scrollablePanel);
            ...
            validAchievements.forEach((achievement) => {
                ...
                if (achievement.unlocked == false) {
                    // Achievement chưa mở khóa - hiển thị trạng thái "Locked"
                    ...
                    // Achievement đã mở khóa - hiển thị trạng thái "Unlocked"
            ...
            maskShape = scene.add
                ...
                .setVisible(false);
            container_invite_list.add(maskShape);
...
export function Close() {
    if (!isOpen) return;
    isOpen = false;
    if (container_invite_list) {
        container_invite_list.destroy();
        container_invite_list = null;
    }
