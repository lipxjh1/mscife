...
import { CreateNeuralinkInventoryUpgradeDetail } from "./HomeNeuralinkInventoryUpgradeDetail.js";
...
function CreateNeuralinkInventoryUpgrade(scene) {
    neuralinkInfo = null;
    ...
    centerData.RequestNeuralinkRefining(
        (result) => {
...
export function ActiveNeuralinkInventoryUpgrade(scene, isActive) {
    if (container_account) {
        Destroy();
    } else if (container_account == null && isActive) {
        CreateNeuralinkInventoryUpgrade(scene);
    }
}
...
function CreateItemList(scene) {
    ...
    for (let i = 0; i < neuralinkInfo.length; i++) {
        ...
        container_item.button_view.button.on("pointerdown", function () {
            CreateNeuralinkInventoryUpgradeDetail(scene, indexData);
        });
    }
...
function CreateItem(scene, scrollablePanel, itemData) {
    ...
    let secondLeft = calculateRemainingSeconds(item.itemData.completionDate);
        ...
        if (seconds > 0) {
            item.text_next_time.setText(
                cdLocalization.getLocalization(
                    cdLocalization.GROUP_KEYS.Neuralink.KEY,
                    "Completed in: "
                ) + calculateTimeRemaining(item.itemData.completionDate)
            );
        } else {
            item.text_next_time.setText(
                cdLocalization.getLocalization(
...
function formatSecondsToHMS(totalSeconds) {
    ...
    if (
        ...
        return "00:00:00"; // Hoặc ném lỗi tùy thuộc vào yêu cầu
    }
