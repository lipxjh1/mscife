...
export function CreateHomeInventorySellNeuralinkPopupInput(
    scene,
    code,
    msciPrice,
    onSuccess,
    ...
    btn_yes.button.on("pointerdown", function () {
        let neuralinkToSell = Number(inputValue);

        if (neuralinkToSell != null && neuralinkToSell > 0) {
            if (neuralinkToSell > itemInventoryData.quantity) {
                CreateAlertPopup(
                    scene,
                    cdLocalization.getLocalization(
                        cdLocalization.GROUP_KEYS.Neuralink.KEY,
            ...
            let caculatedMSCI = neuralinkToSell * msciPrice;

            CreateAlertPopup(
                scene,
                cdLocalization.getLocalization(
                    cdLocalization.GROUP_KEYS.Neuralink.KEY,
                    `Sell {i} Neuralink for {i} $MSCI`,
                    [neuralinkToSell, caculatedMSCI]
                ),
                () => {
                    CreateLoadingPopup();
                    centerData.RequestNeuralinkLiquidate(
                        itemInventoryData._id,
                        neuralinkToSell,
                        (result) => {
                            HideLoadingPopup();
                            itemInventoryData.quantity -= neuralinkToSell;

                            if (onSuccess && typeof onSuccess === "function") {
                                onSuccess(result, neuralinkToSell);
                            }
