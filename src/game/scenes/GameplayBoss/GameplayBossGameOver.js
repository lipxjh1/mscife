...
//     stats: {
//         damageDealt: 5600,
//         damagePercent: "112.00",
//         isLastHitter: true,
//         battleType: "EliteBoss",
//     },
...
export function CreateGameOver(scene, isVictory = false, data) {
    ...
    if (isVictory) {
        popup_bg.setPosition(-2000, -1920 / 2 + 285);
        ...
        const img_text = scene.add
            .image(540, 112, "gameplay_game_complete_text")
            .setOrigin(0.5, 0);
        ...
        scene.tweens.add({
            ...
            y: 112, // Chuyển từ -100 (phía trên màn hình) đến centerY (giữa màn hình)
        ...
        if (data.defeatedBy) {
            ...
            const img_avatar = scene.add
                .image(233, 503, data.defeatedBy.avatar)
                .setDisplaySize(145, 176)
                .setOrigin(0.5, 0);

            container_main.add(img_avatar);

            const text_name = scene.add
                .text(
                    233,
                    679,
                    data.defeatedBy.userId + "\n" + data.defeatedBy.username,
                    {
                        fontFamily: cdLocalization.getCurrentFont(),
                        fontSize: "48px",
                        color: "#ffffff",
                ...
                .setOrigin(0.5, 0);
            container_main.add(text_name);
        }

        centerData.RequestPostBossBattleDefeatedRewards(
            data.battleId,
            (result) => {
                rewardData = result;
                ...
                CreateBattleResult(
                    scene,
                    rewardData.stats.damageDealt,
                    rewardData.stats.damagePercent,
                    rewardData.stats.battleType
                );
        ...
        centerData.RequestBossBattlePoolTopDamage(
            data.battleId,
            (result) => {
                topData = result.topDamage;

                CreateTopDamageItemList(scene);
            },
            (error) => {}
        );

        const btn_claim = scene.add
            .image(48 + 476 / 2, 1711 + 86 / 2, "gameplay_game_over_btn_claim")
            .setInteractive({ useHandCursor: true })
            .on("pointerdown", (pointer) => {
                centerData.RequestPostBossBattleDefeatedRewardsClaim(
                    data.battleId,
                    (result) => {
                        CreateAlertPopup(
                            scene,
        ...
        container_main.add(btn_claim);

        const btn_exit = scene.add
            .image(564 + 476 / 2, 1711 + 86 / 2, "gameplay_game_over_btn_exit")
            .setInteractive({ useHandCursor: true })
            .on("pointerdown", (pointer) => {
                scene.scene.start("Home");
            })
...
function CreateList(scene) {
    ...
    let itemCount = rewardData.rewards.items.length;

    if (rewardData.rewards.chip) {
        itemCount += 1;
    }
...
function CreateItemList(scene, scrollablePanel) {
    ...
    chip.text_value.setText("x" + rewardData.rewards.chip);
    chip.icon
        .setTexture("home_top_currency_chip_1")
        .setOrigin(0.5, 0.5)
        .setDisplaySize(140, 140);

    for (let i = 0; i < rewardData.rewards.items.length; i++) {
        let itemData = rewardData.rewards.items[i];
