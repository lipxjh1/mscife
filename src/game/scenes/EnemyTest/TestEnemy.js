...
export function EnemyIdToKeySpine(enemyId) {
    ...
    // Trong chế độ test, chỉ sử dụng các spine đã được tải trong "earth" range
    if (enemyId === "normal") {
        return "gameplay_enemy_0"; // Đã được tải trong LoadEnemyByRange("earth")
    }

    if (enemyId === "tank") {
        return "gameplay_enemy_1"; // Đã được tải trong LoadEnemyByRange("earth")
    }
...
class TestEnemy {
    constructor(
        ...
        this.spriteKey = EnemyIdToKeyImage(this.enemyType);
        this.spineKey = EnemyIdToKeySpine(this.enemyType);
        this.currentHp = hp;
        this.maxHp = hp;
        ...
        // Hiển thị shield bar nếu kẻ địch có shield
        if (this.maxShield > 0) {
    ...
    setShield(scene, pointer, setShield) {
        ...
        // Cập nhật shield bar khi shield thay đổi
        if (this.maxShield > 0) {
            if (IsShieldBarActive() == false) {
                ActiveShieldBar(true);
            }
            UpdateShieldBar(scene, this.maxShield, this.currentShield);
        }
...
export default TestEnemy; // Đảm bảo sử dụng export default
