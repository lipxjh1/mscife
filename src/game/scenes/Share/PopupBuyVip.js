...
export function CreateBuyVipPopup(scene, priceMusk, onSuccess, onFailed) {
    let inputValue = null;
    ...
    let askStr = cdLocalization.getLocalization(
        ...
        `Do you want to buy Vip with 100 M-Coin/month`
    );
    ...
    btn_yes.button.on("pointerdown", function () {
        let buyMonth = Number(inputValue);

        let caculatedMusk = priceMusk * buyMonth;

        if (priceMusk > 0 && caculatedMusk > centerData.userInfo.Musk) {
            CreateAlertPopup(
                scene,
                cdLocalization.getLocalization(
                    cdLocalization.GROUP_KEYS.HomeShop.KEY,
        ...
        if (buyMonth != null && buyMonth > 0) {
            CreateLoadingPopup();

            centerData.RequestBuyVip(
                buyMonth,
                (result) => {
                    HideLoadingPopup();
