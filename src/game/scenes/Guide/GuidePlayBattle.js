...
export function CreateGuidePlayBattle(scene) {
    if (
    ...
    const text_content = scene.add
        .text(
            ...
            cdLocalization.getLocalization(
                cdLocalization.GROUP_KEYS.HomeBattle.KEY,
                "The campaign game mode will take you to battles that not only help you improve your skills but also come with attractive rewards upon completion or reaching an achievement milestone."
            ),
            {
                fontFamily: cdLocalization.getCurrentFont(),
                fontSize: "28px",
...
export function DestroyGuidePlayBattle(scene) {
    if (container) {
        container.destroy();
    }
}
...
function CreateText(scene, container) {
    ...
    const text = scene.add
        .text(0, 0, "Click the button to play the campian mode", {
            fontFamily: cdLocalization.getCurrentFont(),
            fontSize: "32px",
            color: "#ffffff",
            align: "right",
    ...
    container_text.setPosition(750, 560);
